  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 container-custom text-center py-32 lg:py-40">
        ...
        {/* Main Title */}
        <h1 className={`font-serif text-3xl sm:text-4xl md:text-5xl lg:text-[5.5rem] xl:text-[6.5rem] text-white leading-[1.1] md:leading-[1.05] tracking-wide transition-all duration-1000 ease-out ${phase >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '0.3s' }}>
      ...
      {heroConfig.stats.length > 0 && (
        <div className={`absolute bottom-8 sm:bottom-20 left-0 right-0 z-10 transition-all duration-1000 ease-out ${phase >= 4 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="container-custom px-4">
            <div className={`grid gap-4 sm:gap-8 max-w-3xl mx-auto ${heroConfig.stats.length === 2 ? 'grid-cols-2' : heroConfig.stats.length === 3 ? 'grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4'}`}>
              {heroConfig.stats.map((stat, index) => (
                <div key={index} className={`text-center ${index > 0 ? 'border-l border-white/20' : ''}`}>
                  <div className="font-serif text-xl sm:text-3xl md:text-4xl text-gold-500 mb-1 sm:mb-2 tabular-nums">
      ...
      {/* Bottom fade */}
      <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#0d1525] to-transparent" />
